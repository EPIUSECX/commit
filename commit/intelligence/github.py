...
@contextmanager
def git_auth_environment(installation_id=None):
    """Provide Git with a non-interactive, temporary GitHub App credential."""
    settings = frappe.get_single("Github Settings")
    configured = bool(
        settings.get_password("installation_token", raise_exception=False)
        or (
            settings.github_app_id
            and settings.installation_id
...
def installation_token(installation_id=None):
    settings = frappe.get_single("Github Settings")
    # installation_token is a backwards-compatible manual override. A manifest-
    # created GitHub App normally leaves it empty and mints short-lived tokens.
    configured = settings.get_password("installation_token", raise_exception=False)
    if configured:
        return configured
    ...
    if not private_key or not settings.github_app_id or not installation_id:
        frappe.throw("Configure the GitHub App ID, installation ID, and private key")
    response = requests.post(
        f"{GITHUB}/app/installations/{installation_id}/access_tokens",
...
def github_request(method, path, **kwargs):
    ...
    headers.update({"Authorization": f"Bearer {installation_token(installation_id)}", "Accept": "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28"})
...
def publish_check(project_branch, head_sha):
    ...
    project = frappe.get_doc("Commit Project", branch.project)
    org, installation_id = frappe.db.get_value("Commit Organization", project.org, ["github_org", "github_installation_id"])
    return github_request("POST", f"/repos/{org}/{project.repo_name}/check-runs", installation_id=installation_id, json=check_payload(branch, snapshot, head_sha))
...
def create_docs_pull_request(project_branch, export_result):
    ...
    base_ref = github_request("GET", f"/repos/{org}/{project.repo_name}/git/ref/heads/{project.default_branch or 'main'}", installation_id=installation_id)
    github_request("POST", f"/repos/{org}/{project.repo_name}/git/refs", installation_id=installation_id, json={"ref": f"refs/heads/{new_branch}", "sha": base_ref["object"]["sha"]})
    for path in export_result["files"]:
        content = base64.b64encode(export_result["contents"][path].encode()).decode()
        github_request("PUT", f"/repos/{org}/{project.repo_name}/contents/{path}", installation_id=installation_id, json={"message": f"docs: sync {path} from Commit", "content": content, "branch": new_branch})
    return github_request("POST", f"/repos/{org}/{project.repo_name}/pulls", installation_id=installation_id, json={"title": "docs: synchronize documentation from Commit", "head": new_branch, "base": project.default_branch or "main", "body": "Generated by Commit's reviewed docs-as-code workflow."})
