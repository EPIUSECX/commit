...
import { AlertTriangle, ArrowRight, BookOpen, Box, Braces, CheckCircle2, GitCompare, Network, Play, Search, ShieldAlert } from 'lucide-react'
...
const Empty = ({ children }: { children: React.ReactNode }) => <div className="rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground">{children}</div>
    ...
    <div className="min-h-screen bg-muted/20">
      ...
      <main className="mx-auto max-w-7xl p-4 md:p-6">
        <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div><p className="text-sm text-muted-foreground">Project branch</p><h1 className="text-2xl font-semibold">{ID}</h1><p className="text-sm text-muted-foreground">Continuous API governance, architecture, documentation, and verification.</p></div>
          <div className="flex gap-2"><Button asChild variant="outline"><Link to={`/project-viewer/${ID}`}><Braces className="mr-2 h-4 w-4" />API Explorer</Link></Button><Button asChild variant="outline"><Link to="/"><ArrowRight className="mr-2 h-4 w-4 rotate-180" />Projects</Link></Button></div>
        </div>
