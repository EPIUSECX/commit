...
export default function Intelligence() {
  ...
  const scanRunning = overview?.branch_details?.scan_status === 'Running'
  const scanActionLabel = overview?.branch_details?.scan_status === 'Queued' ? 'Retry queued scan' : snapshot ? 'Refresh & scan' : 'Run first scan'
      ...
      <main className="mx-auto max-w-7xl p-4 md:p-6 lg:p-8">
        <div className="mb-5 overflow-hidden rounded-2xl border bg-background shadow-sm">
          <div className="flex flex-col gap-5 p-5 md:flex-row md:items-center md:justify-between md:p-6">
            ...
            <div className="flex flex-wrap gap-2"><Button onClick={refresh} disabled={refreshBranch.loading || scanRunning}>{refreshBranch.loading || scanRunning ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCcw className="mr-2 h-4 w-4" />}{scanActionLabel}</Button><Button asChild variant="outline"><Link to={`/project-viewer/${ID}`}><Braces className="mr-2 h-4 w-4" />API Explorer</Link></Button><Button asChild variant="ghost"><Link to="/"><ArrowRight className="mr-2 h-4 w-4 rotate-180" />Projects</Link></Button></div>
          </div>
        </div>
